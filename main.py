...
async def send_system_message(guild, member, message):
    try:
        ...
        print(
            f"Cannot send message in system channel for {guild.name}: Missing permissions."
        )
    ...
    for guild in bot.guilds:
        ...
        if not is_valid_server(guild):
            ...
            print(
                ...
                f"Reason: Less than 7 members, Over 300 members, or invalid name)"
            )
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if not discord_json or discord_json.get("status") == "FAILED":
            ...
            await send_system_message(
                member.guild, member, f"{error_message} {member.mention}의 인증에 실패했습니다."
            )
            return
        ...
        try:
            ...
            await send_system_message(
                member.guild, member, f"{member.mention}의 닉네임을 변경할 권한이 없습니다."
            )
        ...
        if verified_role:
            try:
                ...
                await send_system_message(
                    member.guild, member, f"{member.mention}에게 역할을 지급할 권한이 없습니다."
                )
        else:
            await send_system_message(
                member.guild,
...
async def help_command(interaction: discord.Interaction):
    if not is_valid_server(interaction.guild):
        ...
        print(
            f"Left guild: {interaction.guild.name} (Members: {interaction.guild.member_count}, Reason: Less than 7 members, Over 300 members, or invalid name)"
        )
        return
...
async def resident_command(interaction: discord.Interaction, name: str):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        resident_data = await handle_api_response(
            interaction, resident_json, "존재하지 않는 플레이어입니다!"
        )
        ...
        if resident_data.get("town"):
            town_json = await fetch_json(
                session, "town", {"name": resident_data["town"]}
            )
            town_data = await handle_api_response(
                interaction, town_json, "마을 정보를 가져오는 데 실패했습니다."
            )

    embed = discord.Embed(
        title=resident_data["name"].replace("_", "\\_"), color=discord.Color.green()
    )
    embed.set_thumbnail(
        url=f"https://mc-heads.net/avatar/{resident_data['name']}/600.png"
    )
    embed.add_field(
        name="**최초 접속일**",
    ...
    embed.add_field(
        name="**마을**",
        value=resident_data["town"].replace("_", "\\_")
    ...
    embed.add_field(
        name="**국가**",
        value=town_data["nation"].replace("_", "\\_")
...
async def town_command(interaction: discord.Interaction, name: str):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        town_data = await handle_api_response(interaction, town_json, "존재하지 않는 마을입니다!")
        if not town_data:
            return

    embed = discord.Embed(
        title=town_data["name"].replace("_", "\\_"), color=discord.Color.green()
    )
    embed.add_field(
        name="**공지**", value=town_data["townBoard"].replace("_", "\\_"), inline=False
    )
    embed.add_field(
        name="**시장**", value=town_data["mayor"].replace("_", "\\_"), inline=False
    )
    embed.add_field(
        name="**국가**",
        value=town_data["nation"].replace("_", "\\_") if town_data["nation"] else "없음",
        inline=False,
    )
...
async def nation_command(interaction: discord.Interaction, name: str):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        nation_data = await handle_api_response(
            interaction, nation_json, "존재하지 않는 국가입니다!"
        )
        if not nation_data:
            return

    embed = discord.Embed(
        title=nation_data["name"].replace("_", "\\_"), color=discord.Color.green()
    )
    embed.add_field(
        name="**공지**",
        value=nation_data["nationBoard"].replace("_", "\\_"),
        inline=False,
    )
    embed.add_field(
        name="**왕**", value=nation_data["leader"].replace("_", "\\_"), inline=False
    )
    embed.add_field(
        name="**국민 수**", value=str(nation_data["memberCount"]), inline=False
    )
    embed.add_field(
        name="**마을**", value=nation_data["towns"].replace("_", "\\_"), inline=False
    )
    embed.add_field(
        name="**동맹**",
        value=nation_data["allies"].replace("_", "\\_")
        if nation_data["allies"]
        else "없음",
        inline=False,
    )
    embed.add_field(
        name="**적**",
        value=nation_data["enemies"].replace("_", "\\_")
        if nation_data["enemies"]
        else "없음",
        inline=False,
    )
    embed.add_field(
